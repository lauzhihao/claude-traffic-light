...
│   ├── RelayConfig.swift                   # ⚠️ 写死的 agent 地址/密钥，加入两个 target 的 Membership
...
> 本目录是真机工程（`~/Documents/ClaudeTrafficLight`，独立 git 仓库）的源码快照,只收 Swift 文件;Assets / Info.plist / entitlements 等 Xcode 工程文件不入快照。`Factory/` 是同一个 App 里的另一条产品线(ncds 内容工厂前端),与红绿灯功能无耦合,但 `ContentView` 嵌入了它的 `AgentBoard`,快照若不带它将无法编译。

> Mac agent 的地址（`.local` Bonjour 名 + Tailscale IP）和密钥已经写死在 `RelayConfig.swift` 里，App 装上即用、**不用在界面里填任何配置**。换机器或轮换密钥时只改这一个文件再重新编译。推送本身由 Apple APNs 下发（agent 直推），这些地址只用于「注册 token」和「开屏读一次状态」。
