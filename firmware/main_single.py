# Claude 红绿灯 · 单 RGB 灯版(适合 RP2040-Zero · 黄推理/红等你)
# 一颗 WS2812 切色显示状态:R=黄呼吸 Y=红快闪 G=绿常亮 0=灭
...
DATA_PIN = 0               # 数据脚 GP0(外接单颗 WS2812);RP2040-Zero 板载灯用 16
NUM_LEDS = 1
...
BLINK_PERIOD = 0.7         # 快闪周期(s,用于等你=红闪)
BLINK_DUTY = 0.6           # 闪烁亮态占比
...
OFF = (0, 0, 0)
# 配色对调:R(推理)=黄 / Y(等你)=红 / G(完成)=绿(和三灯版一致)
BASE = {'R': AMBER, 'Y': RED, 'G': GREEN, '0': OFF}
...
while True:
    ...
    if trans_from is not None:
        ...
        render(scale(AMBER, breath_f(phase)))      # 推理:黄呼吸
    elif state == 'Y':
        phase = (phase + dt / BLINK_PERIOD) % 1.0
        render(RED if phase < BLINK_DUTY else OFF)  # 等你:红闪
    # G / 0:静态,过渡末已渲染,保持不动
