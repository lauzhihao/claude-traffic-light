...
- `gen.py` → 用 SVG 程序化生成图标,`rsvg-convert` 渲染、ImageMagick 去 alpha。
- `AppIcon-1024.png`:珊瑚径向底 + 奶油星芒(Claude 风格**原创**)+ 右下角绿色状态圆点,已装进主 App 的 `Assets.xcassets/AppIcon.appiconset`(单张 1024 通用)。
- `IconBase-1024.png`:同款但**不带圆点**,装进 Widget 的 `Assets.xcassets/IconBase.imageset`,给主屏组件当背景。

**主屏 Widget**(`ClaudeTrafficLightWidget/StatusIconWidget.swift`,已加进 `WidgetBundle`):`systemSmall`,底图 + SwiftUI 动态圆点(R/Y/G),数据由 Widget 自己定时拉中继 `/health` 的 `latest.state`。
