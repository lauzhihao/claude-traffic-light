...
# 公开版:配了中继(CLAUDE_LIGHT_RELAY_URL + CLAUDE_LIGHT_API_TOKEN)就把状态上报给
# 中继(中继持 .p8 推 APNs),不再本机直推。两模式互斥:配了走中继,否则走本地 apns
# (自托管单机)。中继模式纯 stdlib(urllib),无需 .venv/cryptography。
RELAY_URL = os.environ.get("CLAUDE_LIGHT_RELAY_URL", "").rstrip("/")
RELAY_TOKEN = os.environ.get("CLAUDE_LIGHT_API_TOKEN", "")
# 显式绕开代理:中继是直连可达的公网 IP(自己服务器),别走残留 HTTPS_PROXY。
_RELAY_OPENER = urllib.request.build_opener(urllib.request.ProxyHandler({}))
...
def quota_loop():
    ...
    if not (apns.enabled() or relay_mode()):
        return  # 配额只随 iOS 推送下发;本地推送/中继都没配就不扫,省得反复读一堆 jsonl
    while True:
        try:
...
def push_state(state, priority=10):
    """把状态推到手机 Live Activity。

    配了中继 → 上报中继(中继持 .p8 推 APNs);否则 → 本机直推 APNs。
    priority 透传(10=即时/leading,5=合并省预算/trailing)。两条都带 2 次重试
    (1s/3s 退避)+ light_loop 周期重推兜底,应对单次丢失导致"灯卡色"。
    """
    if state not in ("R", "Y", "G"):
        return
    if relay_mode():
        _push_relay(state, priority)
    elif apns.enabled():
        _push_local(state, priority)


def _push_local(state, priority):
...
def _push_relay(state, priority):
    body = json.dumps({"state": state, "priority": priority,
                       "quota": current_quota or None}).encode()
    req = urllib.request.Request(
        f"{RELAY_URL}/v1/state", data=body, method="POST",
        headers={"Content-Type": "application/json",
                 "Authorization": f"Bearer {RELAY_TOKEN}"})
    for attempt, backoff in ((1, 1), (2, 3), (3, 0)):
        try:
            with _RELAY_OPENER.open(req, timeout=5) as r:
                if r.status == 200:
                    return
                print(f"[push_relay] attempt {attempt}: HTTP {r.status}", file=sys.stderr)
        except urllib.error.HTTPError as e:
            print(f"[push_relay] attempt {attempt}: HTTP {e.code}", file=sys.stderr)
            if e.code in (401, 403, 400):
                return   # 鉴权/请求错,重试无意义
        except Exception as e:
            print(f"[push_relay] attempt {attempt}: {e}", file=sys.stderr)
        if backoff:
            time.sleep(backoff)
    ...
    def do_GET(self):
        ...
        if self.path == "/health":
            ...
            self._respond_json({
                ...
                "push": ("relay:" + RELAY_URL) if relay_mode() else "local-apns",
